...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleUnreadableMessage(HttpMessageNotReadableException ex) {
        // Malformed or missing request body (e.g. invalid JSON) — a client error, not a 500.
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error("Malformed request body.", HttpStatus.BAD_REQUEST, "MALFORMED_REQUEST"));
    }
